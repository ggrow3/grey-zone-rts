...
export interface MatchInfo {
  matchId: string; seed: number; yourTeam: number; players: { username: string; team: number }[];
  turn: number; history: { turn: number; commands: { team: number; payload: string }[] }[]; running: boolean;
}
export interface LobbyState { online: string[]; queue: number; recentGames: import('../api').GameLogEntry[] }
export interface ChatMsg { from: string; text: string; at: string; team?: number }
...
class Hub {
  ...
  private emit(ev: string, ...args: unknown[]) { this.listeners.get(ev)?.forEach(f => f(...args)); }
  ...
  async disconnect() { if (this.conn) { const c = this.conn; this.conn = null; await c.stop().catch(() => {}); } }
  get connected() { return !!this.conn && this.conn.state === HubConnectionState.Connected; }
  ...
  on<T extends unknown[]>(ev: string, fn: (...args: T) => void): () => void {
    ...
    if (!this.listeners.has(ev)) {
      ...
      if (this.conn && !ev.startsWith('$')) this.conn.on(ev, (...args: unknown[]) => this.listeners.get(ev)?.forEach(g => g(...args)));
    }
    this.listeners.get(ev)!.add(f);
    return () => { this.listeners.get(ev)?.delete(f); };
  }
  async invoke<T = void>(method: string, ...args: unknown[]): Promise<T> {
    await this.connect();
    return this.conn!.invoke<T>(method, ...args);
  }
  send(method: string, ...args: unknown[]) { if (this.connected) this.conn!.send(method, ...args).catch(e => console.warn(method, e)); }
}
