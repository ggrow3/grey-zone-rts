...
function statsFor(): [string, string][] {
  ...
  return [[String(Math.round(game.stats.score[PL])) + ' vs ' + Math.round(game.stats.score[EN]), 'Score'], [fmt(game.gameTime), 'Time'], [String(game.stats.kills[PL]), 'Enemy units destroyed'], [String(game.stats.lost[PL]), 'Your units lost'],
    ...
    [PL === UA ? Math.round(game.support) + '%' : String(game.captured[PL]), PL === UA ? 'Support at the end' : 'Enemy trucks captured'], [game.resources.filter(r => r.owner === PL).length + ' of ' + game.resources.length, 'Gas and wheat sites held'], [String(game.tradeTotal[PL]), 'Funds from trade convoys'], [game.stats.salvage[PL] + ' wrecks, ' + game.stats.looted[PL] + ' looted', 'Salvage and loot'],
