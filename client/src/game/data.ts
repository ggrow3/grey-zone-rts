...
export const UNITS: Record<string, UnitDef> = {
  infantry: { label: ['Infantry squad', 'Infantry squad'], shape: 'circle', r: 7, hp: 70, speed: 42, roadMul: 1.3, range: 95, dmg: 7, rof: 0.55,
    cost: 180, crew: 6, time: 12, factory: 'barracks', vision: 170, canCapture: true, operator: true, troop: true, targets: ['inf', 'veh', 'struct'], vsStruct: 0.3, vsVehicle: 0.25, vsInf: 1.1,
  ...
  dprk: { label: ['North Korean infantry', 'North Korean infantry'], shape: 'dprk', r: 7, hp: 90, speed: 42, roadMul: 1.3, range: 95, dmg: 9, rof: 0.5,
    cost: 100, crew: 0, time: 12, factory: 'barracks', vision: 160, canCapture: true, troop: true, targets: ['inf', 'veh', 'struct'], vsStruct: 0.3, vsVehicle: 0.25, vsInf: 1.2,
  ...
  merc: { label: ['International Legion squad', 'Mercenary assault squad'], shape: 'merc', r: 7, hp: 100, speed: 60, roadMul: 1.4, range: 100, dmg: 11, rof: 0.5,
    cost: 260, crew: 0, time: 10, factory: 'barracks', vision: 190, canCapture: true, troop: true, targets: ['inf', 'veh', 'struct'], vsStruct: 0.3, vsVehicle: 0.3, vsInf: 1.25,
  ...
  fireGroup: { label: ['Mobile fire group', 'Mobile fire group'], shape: 'gun', r: 7, hp: 60, speed: 40, roadMul: 1.5, range: 130, dmg: 16, rof: 0.35,
    cost: 140, crew: 3, time: 10, factory: 'barracks', vision: 200, troop: true, targets: ['air'], lowAlt: true, vsAirSmall: 1.35, vsAirLarge: 0.5,
  ...
  moto: { label: ['Motorcycle assault group', 'Motorcycle assault group'], shape: 'moto', r: 7, hp: 40, speed: 105, roadMul: 1.8, range: 70, dmg: 6, rof: 0.4,
    cost: 130, crew: 3, time: 8, factory: 'barracks', vision: 160, canCapture: true, troop: true, targets: ['inf', 'struct'], vsStruct: 0.2, vsInf: 1.2,
    blurb: 'Fast and fragile. Rushes through the kill zone to grab a town before the drones react.' },
  fpv: { label: ['FPV quadcopter', 'FPV quadcopter'], shape: 'tri', r: 6, hp: 30, evade: 0.35, speed: 190, kamikaze: true, dmg: 165, splash: 28, vsVehicle: 1.25, vsStruct: 0.7,
  ...
  fiberFpv: { label: ['Fiber-optic FPV', 'Knyaz Vandal fiber FPV'], shape: 'tritail', r: 6, hp: 30, evade: 0.35, speed: 140, kamikaze: true, dmg: 165, splash: 28, vsVehicle: 1.25, vsStruct: 0.7,
  ...
  mavic: { label: ['Mavic recon quad', 'Mavic recon quad'], shape: 'diamond', r: 6, hp: 60, evade: 0.8, speed: 145, dmg: 0, cost: 40, crew: 0, operated: true, endurance: 110, recharge: 30, electric: true, link: 650, time: 0.25, recon: true,
  ...
  bomber: { label: ['Vampire heavy bomber (Baba Yaga)', 'Heavy bomber hexacopter'], shape: 'hex', r: 9, hp: 140, evade: 0.15, speed: 130, range: 80, dmg: 65, splash: 40, rof: 2.0, vsInf: 1.4, vsStruct: 0.8, vsVehicle: 0.8,
  ...
  howitzer: { label: ['Howitzer', 'Howitzer'], shape: 'pent', r: 11, sizeMul: 1.4, hp: 160, speed: 34, roadMul: 1.4, range: 430, minRange: 110, dmg: 95, splash: 42, rof: 5.5,
    ...
    blurb: 'Long range, slow reload, splash damage. Needs a recon drone to see its targets. Carries 12 shells: ammunition trucks and the artillery depot refill it. Park it in a wood: in the open a gun is seen from far off, every shot shows it to radar for six seconds, and drones hit it 45% harder. Shells splash friend and foe alike: keep your own troops out of the beaten zone.' },
  mlrs: { label: ['Rocket artillery', 'Rocket artillery'], shape: 'wedge', r: 12, sizeMul: 1.45, hp: 170, speed: 50, roadMul: 1.4, range: 620, minRange: 220, dmg: 55, splash: 38, rof: 14, salvo: 6,
    ...
    blurb: 'Six-rocket salvos across most of the map. Long reload. Carries three salvos; trucks bring more. Fire from a wood and move after every mission. Six rockets with wide scatter: your own troops near the target die with the enemy.' },
  ...
  geran: { label: ['Geran-2 (Shahed-136)', 'Geran-2 (Shahed-136)'], shape: 'dart', r: 8, hp: 80, evade: 0.15, speed: 135, kamikaze: true, dmg: 220, splash: 40, vsStruct: 1.2,
...
export const COUNTERS: [string, string][] = [
  ['Infantry', 'beats infantry in the open (cover matters more than numbers); loses to IFVs, bombers, and FPVs; rifles barely scratch armor or buildings'],
  ['IFV', 'beats infantry (autocannon x1.5) and small drones; loses to tanks (its gun does half damage to armor) and to FPVs'],
  ['Tank', 'beats vehicles (x1.4) and buildings (x1.2); cannot shoot drones at all and only splashes infantry (x0.6); seven FPVs kill it'],
  ['Mobile fire group', 'beats small drones (x1.35: FPVs, Mavics, Stings); useless against everything else and reaches nothing high'],
  ['Mobile air defense', 'beats large aircraft (x1.6: Shark, Orlan, bombers, Gerans, Liutyi, Lancet); shoots nothing on the ground; a Lancet target'],
  ['Sting / Yolka interceptor', 'beats slow large aircraft (x1.5, Gerans especially); poor against quads (drones dodge other drones); prey for air defense'],
  ['FPV, fiber FPV', 'beats vehicles (x1.25) and troops in the open; weak against buildings (x0.7); dies to jammers (radio only), nets, fire groups'],
  ['Heavy bomber', 'beats infantry clusters and trenches (x1.4); weak against buildings and vehicles (x0.8); air defense and Stings kill it'],
  ['Howitzer, rockets', 'beats buildings (x1.3) and dug-in infantry (x1.2, rockets x1.3); needs a spotter; helpless against drones; hunted by Lancets and counter-battery radar'],
  ['Lancet', 'beats vehicles (x1.4: guns, air defense, jammers, tanks); weak against infantry (x0.8); air defense and interceptors stop it'],
...
/** a posture a unit can be switched to (R cycles, the selection panel shows the buttons); the first in each set is the default */
export interface ModeDef { key: string; label: string; short: string; desc: string }
export const MODE_SETS: Record<string, ModeDef[]> = {
  fpv: [
    { key: 'hunt', label: 'Hunt', short: '', desc: 'Dives on its own at anything it sees inside its hunting radius' },
    { key: 'hold', label: 'Hold', short: 'HOLD', desc: 'Loiters where it is and dives only on your order: right-click a target, press F, or take the sticks' },
    { key: 'ambush', label: 'Ambush', short: 'AMBUSH', desc: 'Lands where it is with the motors off: no battery drain, seen only within 60, and it pounces on the first enemy that comes within 220. The road ambush' },
  ],
  interceptor: [
    { key: 'patrol', label: 'Patrol', short: '', desc: 'Hunts enemy drones up to 330 out and chases them' },
    { key: 'guard', label: 'Guard', short: 'GUARD', desc: 'Stays over its post, engages only within 170, and comes back: point defense for a substation or a battery' },
  ],
  recon: [
    { key: 'high', label: 'High', short: '', desc: 'Flies high: out of reach of machine guns and sees far, but cannot see into woods and trenches' },
    { key: 'low', label: 'Low', short: 'LOW', desc: 'Drops to treetop height: sees troops and guns hidden in woods and trenches anywhere in its view (65% as wide), but machine guns and IFVs can reach it' },
  ],
  jammer: [
    { key: 'emit', label: 'Emitting', short: '', desc: 'Jams enemy radio drones in its bubble; the emission shows it to any enemy radar post within 900' },
    { key: 'silent', label: 'Silent', short: 'SILENT', desc: 'Jammer off: hidden from radar, jams nothing. Switch on when the drones come' },
  ],
  aa: [
    { key: 'active', label: 'Radar on', short: '', desc: 'Full range, and it can intercept glide bombs and missiles; the radar shows it to enemy radar posts within 900 (Lancet bait)' },
    { key: 'passive', label: 'Passive', short: 'PASSIVE', desc: 'Radar off: hidden from radar, range down to 60%, cannot intercept strikes' },
  ],
  arty: [
    { key: 'static', label: 'Stay put', short: '', desc: 'Fires from where it stands' },
    { key: 'scoot', label: 'Shoot and scoot', short: 'SCOOT', desc: 'After every fire mission the gun moves 90 to 150 (into trees when it can) before firing again: counter-battery shells land on empty ground' },
  ],
  troop: [
    { key: 'march', label: 'March', short: '', desc: 'Moves at full speed as a group' },
    { key: 'creep', label: 'Creep', short: 'CREEP', desc: 'Dispersed in twos and threes at 55% speed: in the open seen only within 150 and drones hit 35% less. How to cross a field alive' },
  ],
  armor: [
    { key: 'mobile', label: 'Mobile', short: '', desc: 'Moves and fights normally' },
    { key: 'hullDown', label: 'Hull down', short: 'HULL DOWN', desc: 'Stops in a scrape: takes 30% less damage and reaches 10% farther until it gets a move order' },
  ],
  fireGroup: [
    { key: 'post', label: 'Post', short: '', desc: 'Stays where it is put' },
    { key: 'escort', label: 'Escort trucks', short: 'ESCORT', desc: 'Shadows the nearest friendly truck within 700, guns up. Drones love trucks' },
  ],
};
export const MODE_SET_OF: Record<string, string> = { fpv: 'fpv', fiberFpv: 'fpv', interceptor: 'interceptor', mavic: 'recon', jammer: 'jammer', aa: 'aa', howitzer: 'arty', mlrs: 'arty', infantry: 'troop', merc: 'troop', dprk: 'troop', defector: 'troop', tank: 'armor', ifv: 'armor', fireGroup: 'fireGroup' };
export function modesOf(type: string): ModeDef[] | null { const s = MODE_SET_OF[type]; return s ? MODE_SETS[s] : null; }
/** a human on the sticks (Y): the drone flies toward the cursor; it dodges more, hits harder, and flies faster while piloted */
export const PILOT = { evade: 0.15, dmg: 1.2, speed: 1.15, hold: 0.45, ambushReach: 220, guardReach: 170 };
