...
export function poly(t: CanvasRenderingContext2D, pts: LatLon[], close: boolean) {
  t.beginPath();
  pts.forEach(([lat, lon], i) => { const p = geo(lat, lon); if (i) t.lineTo(p.x, p.y); else t.moveTo(p.x, p.y); });
  if (close) t.closePath();
}
export function outlinedText(t: CanvasRenderingContext2D, text: string, x: number, y: number, size: number, color: string, italic?: boolean) {
...
export function drawPipelines(c: CanvasRenderingContext2D) {
...
const TILE_Z = 11;
const TILE_SRC: Record<Exclude<BasemapMode, 'drawn'>, { url: (z: number, x: number, y: number) => string; attrib: string }> = {
...
function lat2tile(lat: number, z: number) { const r = lat * Math.PI / 180; return (1 - Math.log(Math.tan(r) + 1 / Math.cos(r)) / Math.PI) / 2 * Math.pow(2, z); }
...
export class TerrainCanvas {
  terrain: HTMLCanvasElement; mm: HTMLCanvasElement;
  basemapMode: BasemapMode = 'drawn'; basemap: HTMLCanvasElement | null = null; basemapTiles = 0; basemapTotal = 0; basemapFailed = 0; private basemapGen = 0;
  ...
  constructor() {
    ...
    this.mm = document.createElement('canvas'); this.mm.width = MM_W; this.mm.height = MM_H;
    this.mm.getContext('2d')!.drawImage(this.terrain, 0, 0, MM_W, MM_H);
  }

  scorch(x: number, y: number, r: number) { scorch(this.terrain.getContext('2d')!, x, y, r); if (this.basemap) scorch(this.basemap.getContext('2d')!, x, y, r); }

  attrib(): string { return this.basemapMode === 'drawn' ? '' : TILE_SRC[this.basemapMode].attrib; }

  loadBasemap(mode: BasemapMode) {
    this.basemapMode = mode;
    if (mode === 'drawn') { this.basemap = null; return; }
    this.basemap = document.createElement('canvas'); this.basemap.width = W; this.basemap.height = H_LAND;
    const b = this.basemap.getContext('2d')!, z = TILE_Z, src = TILE_SRC[mode], gen = ++this.basemapGen;
    this.basemapTiles = 0; this.basemapFailed = 0;
    ...
    for (let tx = x0; tx <= x1; tx++) for (let ty = y0; ty <= y1; ty++) {
      ...
      img.onerror = () => { if (gen !== this.basemapGen) return; this.basemapFailed++; if (this.basemapFailed === 3) this.onMessage('Map tiles could not be loaded'); };
