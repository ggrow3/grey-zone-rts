...
import { UNITS, STRUCTS, HOTKEYS, drawR, MODE_SET_OF, MODE_SETS } from './data';
...
export class Controller {
  ...
  set amoveMode(v: boolean) {
    this.view.amoveMode = v;
  }
  private idleIdx = 0;
  ...
  onToggle: (panel: 'tech' | 'manual' | 'pause' | 'legend' | 'audio' | 'log' | 'speed') => void = () => {};
  ...
  set selection(v: Entity[]) {
    this.view.selection = v;
  ...
  // ---- camera
  resize(vw: number, vh: number, dpr: number) {
    this.view.vw = vw;
    this.view.vh = vh;
    this.view.dpr = dpr;
    this.clampCam();
  ...
  toWorld(mx: number, my: number) {
    const { cam } = this.view;
    return { x: mx / cam.z + cam.x, y: my / cam.z + cam.y };
  }
  zoomAt(factor: number, mx: number, my: number) {
    const { cam } = this.view,
      before = this.toWorld(mx, my);
    cam.z = clamp(cam.z * factor, 0.25, 2.5);
    cam.x = before.x - mx / cam.z;
    cam.y = before.y - my / cam.z;
    this.clampCam();
  }
  ...
  goHome() {
    const hq = this.game.hq(this.team);
    if (hq) this.centerOn(hq.x, hq.y);
  }
  handleCamera(dt: number) {
    const { cam, vw, vh, mouse, drag } = this.view,
      k = this.keys;
    if (this.view.pilot) {
      this.flyPilot(dt);
      return;
    ...
    if (k.ArrowLeft || k.KeyA) cam.x -= sp;
    if (k.ArrowRight || k.KeyD) cam.x += sp;
    if (k.ArrowUp || k.KeyW) cam.y -= sp;
    if (k.ArrowDown || k.KeyS) cam.y += sp;
  ...
  prune() {
    ...
    const v = this.view;
    if (v.pilot && (v.pilot.dead || v.pilot.grounded || v.pilot.landed))
      this.releasePilot(
        v.pilot.dead
          ? v.pilot.def.kamikaze && v.pilot.hp > 0
            ? 'Impact. Sticks handed back.'
            : 'Drone lost. Sticks handed back.'
          : 'The drone is down for charging: sticks handed back'
      );
    if (v.pilotTarget && v.pilotTarget.dead) {
      v.pilotTarget = null;
      if (v.pilot) this.onMessage('Target destroyed');
    }
  }
  ...
  togglePilot() {
    ...
    const u = this.selUnits().find(
      x =>
        ...
        !x.landed &&
        !x.dead &&
        !(g.needsOperator(x.def, x.team) && (!x.operator || x.operator.dead))
    );
    if (!u)
      return this.onMessage(
        this.selUnits().some(x => x.def.air)
          ? 'That drone cannot take a pilot right now: it is grounded, charging, or has no squad'
          : 'Select an airborne drone first (Y)'
      );
    const v = this.view;
    v.pilot = u;
    v.pilotTarget = null;
    v.pilotDive = null;
    this.selection = [u];
    this.zoomBefore = v.cam.z;
    if (v.cam.z < 1.4) v.cam.z = 1.4;
    v.placing = null;
    v.bombardMode = false;
    ...
    this.steerT = 0.1;
    this.onMessage(
      'You have the sticks of the ' +
        u.def.label[u.team] +
        ': it flies to your cursor. Left-click ' +
        (u.def.kamikaze ? 'a target to attack, or the ground to dive on it' : 'a target to attack') +
        '. Y or Esc hands it back.'
    );
    this.onSelectionChange();
  }
  ...
  private flyPilot(dt: number) {
    ...
    this.steerT = 0.1;
    if (v.pilotTarget && !v.pilotTarget.dead) {
      this.submit({ kind: 'steer', id: u.id, x: v.pilotTarget.x, y: v.pilotTarget.y, targetId: v.pilotTarget.id });
      return;
    }
    if (v.pilotDive) {
      this.submit({ kind: 'steer', id: u.id, x: v.pilotDive.x, y: v.pilotDive.y, dive: true });
      return;
    }
    if (!v.mouse.inside) {
      this.submit({ kind: 'steer', id: u.id, x: u.x, y: u.y });
      return;
    }
  ...
  private pilotClick(x: number, y: number) {
    ...
    if (t) {
      if (!this.game.canHitTarget(u, t))
        return this.onMessage('This drone cannot hit that: ' + (t.isUnit ? t.def.label[t.team] : t.def.label));
      if (!this.game.inLink(u, t))
        return this.onMessage("Beyond the squad's control range: move the squad closer or pick a nearer target");
      v.pilotTarget = t;
      v.pilotDive = null;
      this.steerT = 0;
      this.onMessage('Attacking the ' + (t.isUnit ? t.def.label[t.team] : t.def.label));
    } else if (u.def.kamikaze) {
      v.pilotDive = { x, y };
      v.pilotTarget = null;
      this.steerT = 0;
      this.onMessage('Diving on the point: right-click to pull up');
    }
  }
  ...
  setMode(mode: string, units?: Unit[]) {
    const list = (units || this.selUnits()).filter(u => {
      const s = MODE_SET_OF[u.type];
      return s && MODE_SETS[s].some(m => m.key === mode);
    });
  ...
  cycleMode() {
    const g = this.game,
      sets = new Map<string, Unit[]>();
    for (const u of this.selUnits()) {
      const s = MODE_SET_OF[u.type];
      if (s) (sets.get(s) || sets.set(s, []).get(s)!).push(u);
    }
    if (!sets.size) return this.onMessage('Nothing selected has postures to switch (R)');
    for (const [s, list] of sets) {
      const defs = MODE_SETS[s],
        i = defs.findIndex(m => m.key === g.modeOf(list[0]));
      this.submit({ kind: 'mode', ids: this.ids(list), mode: defs[(i + 1) % defs.length].key });
    }
  }
  ...
  mouseDown(px: number, py: number, button: number, shift: boolean) {
    ...
    if (button === 0) {
      if (this.view.pilot) {
        this.pilotClick(w.x, w.y);
        return;
      }
      if (this.strikeMode) {
        const m = this.strikeMode;
        this.strikeMode = null;
        if (m === 'kab') this.submit({ kind: 'kab', x: w.x, y: w.y });
        else {
          const t = this.findEnemyAt(w.x, w.y);
          if (t && t.isStruct) this.submit({ kind: 'iskander', targetId: t.id });
          else this.onMessage('Missiles need an enemy building: click one');
        }
        this.onSelectionChange();
        return;
      }
      if (this.view.bombardMode) {
        this.view.bombardMode = false;
        this.bombardAt(w.x, w.y);
        return;
      }
      if (this.amoveMode) {
        this.amoveMode = false;
        this.attackMove(w.x, w.y, shift);
        this.onSelectionChange();
        return;
      }
      if (this.view.placing) {
        this.tryPlace(w.x, w.y, shift);
        return;
      }
      this.view.drag = { x0: w.x, y0: w.y, x1: w.x, y1: w.y };
      (this.view.drag as { shift?: boolean }).shift = shift;
    } else if (button === 1) {
      this.pan = { mx: px, my: py, cx: this.view.cam.x, cy: this.view.cam.y };
    }
  }
  mouseMove(px: number, py: number) {
    ...
    if (this.view.drag) {
      const w = this.toWorld(px, py);
      this.view.drag.x1 = w.x;
      this.view.drag.y1 = w.y;
    }
    if (this.pan) {
      this.view.cam.x = this.pan.cx - (px - this.pan.mx) / this.view.cam.z;
      this.view.cam.y = this.pan.cy - (py - this.pan.my) / this.view.cam.z;
      this.clampCam();
    }
  }
  mouseUp(button: number) {
    ...
    if (button === 0 && this.view.drag) {
      const d = this.view.drag as { x0: number; y0: number; x1: number; y1: number; shift?: boolean };
      this.view.drag = null;
  ...
  doubleClick(px: number, py: number) {
    ...
    for (const u of g.units)
      if (u.team === PL && !u.dead && !u.def.auto) {
        const dd = dist(parallaxOf(u, this.view), w) + (u.def.air ? 3 : 0);
        if (dd <= drawR(u.def) + 8 && dd < bd) {
          bd = dd;
          hit = u;
        }
      }
    const picked = g.units.filter(
      u => u.team === PL && !u.dead && !u.def.auto && onScreen(u) && (hit ? u.type === hit.type : u.def.air)
    );
    if (!picked.length) return this.onMessage('No drones on screen');
    this.selection = g.expandSwarms(picked);
    this.view.drag = null;
    this.onMessage(
      picked.length + ' ' + (hit ? UNITS[hit.type].label[PL] : 'drone') + (picked.length > 1 ? 's' : '') + ' selected'
    );
  }
  contextMenu(px: number, py: number, ctrl: boolean, shift = false) {
    if (this.view.pilot) {
      const v = this.view;
      if (v.pilotTarget || v.pilotDive) {
        v.pilotTarget = null;
        v.pilotDive = null;
        this.steerT = 0;
        this.onMessage('Let go: flying to the cursor');
      }
      return;
    }
    if (this.strikeMode) {
    ...
    if (this.view.bombardMode) {
      this.view.bombardMode = false;
      return this.bombardAt(w.x, w.y);
    }
    this.issueCommand(w.x, w.y, ctrl, shift);
  }
  wheel(deltaY: number, px: number, py: number) {
    this.zoomAt(Math.exp(-deltaY * 0.0012), px, py);
  }
  minimapDown(fx: number, fy: number, button: number) {
    const wx = fx * W,
      wy = fy * H;
    if (button === 2) {
      this.issueCommand(wx, wy, false);
      return;
    }
    this.centerOn(wx, wy);
  }
  ...
  clickSelect(x: number, y: number, shift: boolean) {
    ...
    for (const u of g.units)
      if (u.team === PL && !u.dead && !u.def.auto) {
        const dd = dist(parallaxOf(u, this.view), { x, y }) + (u.def.air ? 3 : 0);
        if (dd <= drawR(u.def) + 5 && dd < bd) {
          bd = dd;
          best = u;
        }
      }
    ...
    if (!best) {
      if (!shift) this.selection = [];
      else this.onSelectionChange();
      return;
    }
    let sel = this.view.selection;
    if (shift) {
      const i = sel.indexOf(best);
      if (i >= 0) sel = sel.filter(e => e !== best);
      else if (best.isUnit) sel = sel.filter(e => e.isUnit).concat([best]);
      else sel = [best];
    } else sel = [best];
  ...
  boxSelect(x0: number, y0: number, x1: number, y1: number, shift: boolean) {
    const g = this.game,
      PL = this.team;
    const found = g.units.filter(
      u => u.team === PL && !u.dead && !u.def.auto && u.x >= x0 && u.x <= x1 && u.y >= y0 && u.y <= y1
    );
    let sel: Entity[];
    if (!found.length && !shift) sel = [];
    else if (shift) {
      const base = this.view.selection.filter(e => e.isUnit) as Unit[];
  ...
  findEnemyAt(x: number, y: number): Entity | null {
    ...
    let best: Entity | null = null,
      bd = Infinity;
    for (const u of g.units)
      if (u.team === EN && !u.dead && u.seenBy[PL]) {
        const dd = dist(parallaxOf(u, this.view), { x, y });
        if (dd <= drawR(u.def) + 7 && dd < bd) {
          bd = dd;
          best = u;
        }
      }
  ...
  selUnits(): Unit[] {
    return this.view.selection.filter(e => e.isUnit && !e.dead) as Unit[];
  }
  selFactories(): Struct[] {
    return this.view.selection.filter(e => e.isStruct && !e.dead && e.def.produces) as Struct[];
  }
  ...
  issueCommand(x: number, y: number, ctrl: boolean, shift = false) {
    ...
    if (shift && units.length) {
      this.submit({ kind: 'move', ids: this.ids(units), x, y, formation: this.formationType, queue: true });
      return;
    }
    if (!units.length) {
    ...
    if (enemy) {
      this.submit({ kind: 'attack', ids: this.ids(units), targetId: enemy.id });
      return;
    }
  ...
  setFormation(f: FormationType) {
    ...
    const units = this.selUnits();
    const sw =
      units.length && units.every(u => this.game.swarmOf(u) === this.game.swarmOf(units[0]) && this.game.swarmOf(u))
        ? this.game.swarmOf(units[0])
        : null;
    if (sw) this.submit({ kind: 'swarmFormation', swarmId: sw.id, formation: f });
    this.onSelectionChange();
  }
  /** Q: the next click is an attack-move: units fight whatever they meet on the way and then carry on */
  startAttackMove() {
    const units = this.selUnits().filter(u => u.def.dmg > 0 || u.def.kamikaze);
    if (!units.length) return this.onMessage('Select units that can shoot first');
    this.amoveMode = !this.amoveMode;
    this.view.bombardMode = false;
    this.strikeMode = null;
    if (this.amoveMode)
      this.onMessage(
        'Attack-move: click where to go. They will stop for anything they meet. Right-click or Esc to cancel.'
      );
    this.onSelectionChange();
  }
  attackMove(x: number, y: number, queue = false) {
    const units = this.selUnits();
    if (!units.length) return;
    this.submit({ kind: 'move', ids: this.ids(units), x, y, formation: this.formationType, attackMove: true, queue });
  }
  /** cycle through squads (or drones) that have nothing to do, centring the camera on each */
  selectIdle(air: boolean) {
    const g = this.game,
      PL = this.team;
    ...
    if (!idle.length) return this.onMessage(air ? 'No idle drones' : 'No idle squads');
    const u = idle[this.idleIdx++ % idle.length];
    this.selection = g.expandSwarms([u]);
    this.centerOn(u.x, u.y);
    this.onMessage(
      (air ? 'Idle drone ' : 'Idle squad ') +
        (((this.idleIdx - 1) % idle.length) + 1) +
        ' of ' +
        idle.length +
        ': ' +
        u.def.label[PL]
  ...
  /** Backspace: jump to the latest thing that happened to your side */
  jumpToAlert() {
    const list = this.game.alerts.filter(a => a.team === this.team);
    if (!list.length) return this.onMessage('Nothing to jump to');
    const a = list[list.length - 1];
    this.centerOn(a.x, a.y);
    this.onMessage(a.text);
  }
  startBombard() {
    if (this.selUnits().some(u => u.def.indirect)) {
      this.view.bombardMode = true;
      this.onMessage('Click where the guns should fire. Right-click or Esc to cancel.');
    }
  }
  ...
  /** Shift-click queues up to five */
  enqueue(fac: Struct, type: string, n = 1) {
    const room = Math.max(1, 5 - fac.queue.length);
    for (let i = 0; i < Math.min(n, room); i++) this.submit({ kind: 'enqueue', facId: fac.id, type });
  }
  cancelQueued(fac: Struct, index: number) {
    this.submit({ kind: 'cancel', facId: fac.id, index });
  }
  buyUpgrade(key: string) {
    this.submit({ kind: 'upgrade', key });
  }
  geranWave() {
    this.submit({ kind: 'wave' });
  }
  startStrike(kind: 'kab' | 'iskander') {
    this.strikeMode = this.strikeMode === kind ? null : kind;
    this.onMessage(
      kind === 'kab'
        ? 'Click where the glide bomb should land. Right-click to cancel.'
        : 'Click the enemy building the missile should hit. Right-click to cancel.'
    );
    this.onSelectionChange();
  }
  deepStrike() {
  ...
  keyDown(e: KeyboardEvent): boolean {
    ...
    const v = this.view;
    if (e.code === 'Escape') {
      if (v.pilot) this.releasePilot('Sticks handed back');
      else if (this.amoveMode) this.amoveMode = false;
      else if (this.strikeMode) this.strikeMode = null;
      else if (v.bombardMode) v.bombardMode = false;
      else if (v.placing) v.placing = null;
      else this.view.selection = [];
      this.onSelectionChange();
    } else if (e.code === 'KeyP') this.onToggle('pause');
    else if (e.code === 'KeyQ') this.startAttackMove();
    else if (e.code === 'Period') this.selectIdle(false);
      ...
      if (e.ctrlKey || e.shiftKey) {
        this.groups[k] = this.selUnits();
        this.onMessage('Group ' + k + ' set');
        return true;
      ...
      if (idx >= 0) {
        const fac = this.selFactories();
        if (fac.length === 1) {
          const list = fac[0].def.produces!.filter(k => UNITS[k].side === undefined || UNITS[k].side === this.team);
          if (list[idx]) this.enqueue(fac[0], list[idx]);
        }
      }
    }
    return false;
