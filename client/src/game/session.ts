...
export interface TurnDto {
  turn: number;
  commands: { team: number; payload: string }[];
}
    ...
    while (this.acc >= DT && steps < maxSteps) {
      if (this.pending.length) {
        for (const c of this.pending) {
          this.game.apply(this.myTeam, c);
          this.record.push({ tick: this.tickN, cmd: c });
        }
        this.pending = [];
      }
      this.game.tick(DT);
...
export class ReplaySession implements Session {
  ...
  advance(elapsed: number) {
    ...
    const maxSteps = 15 * this.speed;
    while (this.acc >= DT && steps < maxSteps) {
      while (this.next < this.commands.length && this.commands[this.next].tick === this.tickN) {
        this.game.apply(this.myTeam, this.commands[this.next].cmd);
        this.next++;
      }
      this.game.tick(DT);
      this.acc -= DT;
...
export class NetSession implements Session {
  ...
  constructor(
    public game: Game,
    public myTeam: number,
    private net: NetTransport,
    history: TurnDto[]
  ) {
    for (const t of history) this.receive(t);
  }
  receive(t: TurnDto) {
    this.turns.set(t.turn, t);
    if (t.turn > this.lastReceived) this.lastReceived = t.turn;
  }
  submit(cmd: Command) {
  ...
  get lag(): number {
    return this.lastReceived - this.turnDone;
  }
  advance(elapsed: number) {
    ...
    while (this.acc >= DT && steps < maxSteps) {
      if (this.tickInTurn === 0) {
        ...
        for (const c of next.commands) {
          try {
            this.game.apply(c.team, JSON.parse(c.payload) as Command);
          } catch (e) {
