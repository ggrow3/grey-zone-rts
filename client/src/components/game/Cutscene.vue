...
<template>
  <div id="cutscene" :class="{ ru: side === 1 }" @click="$emit('next')">
    <div class="bars top" /><div class="bars bottom" />
    <div class="card">
      <div class="kicker">{{ side === 0 ? 'Ukrainian Armed Forces, Kharkiv group' : 'Russian Armed Forces, Belgorod group' }} · briefing</div>
      <h1>{{ title }}</h1>
      <div class="lines">
        <p v-for="(l, i) in lines.slice(0, index + 1)" :key="i" :class="{ now: i === index, past: i < index }">{{ l }}</p>
      </div>
      <div class="foot">
        ...
        <button type="button" @click.stop="$emit('skip')">{{ index >= lines.length - 1 ? 'Begin (Enter)' : 'Skip briefing (Esc)' }}</button>
      </div>
    </div>
  </div>
</template>
