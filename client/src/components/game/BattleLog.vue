...
const entries = computed(() => {
  void props.tick;
  const l = props.game.log;
  return props.feed ? l.slice(-6) : l.slice().reverse();
});
</script>

<template>
  <div v-if="feed" id="killfeed">
    <div v-for="(e, i) in entries" :key="e.at + ':' + i" class="m" :class="cls(e)">
      <span class="t">{{ fmtTime(e.at) }}</span> {{ e.text }}
    </div>
  </div>
  <div v-else id="battlelog" class="panel">
    <div class="head">
      <h3>Battle log</h3>
      <button type="button" @click="$emit('close')">Close (K)</button>
    </div>
    <div class="body">
      <div v-for="(e, i) in entries" :key="e.at + ':' + i" class="m" :class="cls(e)">
        <span class="t">{{ fmtTime(e.at) }}</span> {{ e.text }}
