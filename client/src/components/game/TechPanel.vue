...
const nodes = (br: string) =>
  Object.entries(UPGRADES)
    .filter(([, u]) => u.branch === br)
    .sort((a, b) => a[1].tier - b[1].tier);
const state = (key: string) => {
  void props.tick;
  const g = props.ctl.game,
    T = props.ctl.team;
  return g.upgrades[T][key] ? 'owned' : g.upgAvailable(T, key) ? 'avail' : 'locked';
};
</script>
...
<template>
  <div id="techpanel" class="panel" :key="tick">
    <div id="techHead">
      <h3>Research and procurement</h3>
      <span>{{ Math.floor(ctl.game.funds[ctl.team]) }} funds available</span
      ><button type="button" @click="emit('close')">Close (T)</button>
    </div>
    <div id="techGrid">
            ...
            <div class="c">
              {{
                state(key) === 'owned'
                  ...
                  : state(key) === 'avail'
                    ? u.cost + ' funds'
                    : 'Needs ' + upgLabel(ctl.team, u.requires!)
              }}
            </div>
          </button>
        </template>
