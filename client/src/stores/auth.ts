...
export const useAuth = defineStore('auth', () => {
  ...
  function set(t: string, u: string) {
    ...
    username.value = u;
    try {
      sessionStorage.setItem(KEY, JSON.stringify({ token: t, username: u }));
    } catch {
      /* ignore */
    }
  }
  ...
  async function register(u: string, p: string) {
    const r = await post('/api/auth/register', { username: u, password: p });
    set(r.token, r.username);
    await refresh();
  }
  ...
  async function refresh() {
    ...
    completedLevels.value = me.completedLevels || [];
    wins.value = me.wins;
    losses.value = me.losses;
  }
